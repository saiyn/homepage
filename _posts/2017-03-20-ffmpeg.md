			...
			switch(nal->type){
				...
				case H264_NAL_SLICE:
					...
					if(h->current_slice == 1){
						 ...
						 */
						if(h->avctx->hwaccel)
							h->avctx->hwaccel->start_frame(h->avctx, buf, buf_size);	
					...
					if(h->nb_slice_ctx_queued == max_slice_ctx){
						if(h->avctx->hwaccel){
							 ...
							 * 这里只是进行硬件解码器的参数设置。
							 */
							avctx->hwaccel->decoded_slice(avctx, nal->raw_data, nal->raw_size);
							h->nb_slice_ctx_queued = 0;
						}else{
							...
							/**
							 * 如果是软解，调用ff_h264_execute_decode_slice()进行解码。
							 */
						}
					}
				...
				case H264_NAL_PPS:
					 ...
					 * 解析PPS信息。
					 */
					ff_h264_decode_picture_parameter_set(&nal->gb, avctx, &h->ps, nal->size_bits);
					...
				break;
