...
在本地commit一个版本的代码后，经常需要回退到之前commit的某个版本进行代码测试，待测试完成后又需要回到最新的commit版本上继续开发。

执行`git checkout xxxx`回退到想要测试的版本，xxxx是可以区别于其他所有commit号的前几位即可。

![git_10](http://omp8s6jms.bkt.clouddn.com/image/git/git-10.png)

<br />

查看git log可见，执行回退后，较新的commit信息都看不到了。这时要再回到之前最新的commit版本怎么办？

很简单，执行`git reset --soft HEAD@{1}`命令即可。

![git_11](http://omp8s6jms.bkt.clouddn.com/image/git/git-11.png)
