...
* 先交叉编译两个依赖库的代码，假设分别安装在`/opt/readline_install64`和`/opt/ncurse_install64`目录下
* 然后设置lua源码根目录下的build.sh，支持交叉编译环境
* 然后在src/Makefile中添加如下：
```
	MYCFLAGS= -I/opt/readline_install64/include -I/opt/ncurse_install64/include
	MYLIBS= -L/opt/readline_install64/lib -L/opt/ncurse_install64/lib
...
然后执行`docker build -t lua-image .` 创建image, 然后执行`docker run -i -v xxx:/build lua-image bash -c "qemu-arm-static /bin/luac yyy.lua -o yyy"`
